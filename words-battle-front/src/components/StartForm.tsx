...
function Tabs ({mode, onSelectCreate, onSelectJoin} : OptionsProps) {
	return(
		<div className={style.options}>
			<div className={mode === "create"? `${style.option} ${style.selected}` : style.option } onClick={onSelectCreate}>CREATE ROOM</div>
			<div className={mode === "join"? `${style.option} ${style.selected}` : style.option} onClick={onSelectJoin}>JOIN ROOM</div>
		</div>
	);
}
...
function Inputs ({onTypeUsername, onTypeRoomname, username} : InputsProps) {
	return(
		<div className={style.inputs}>
			<label >USERNAME:
				<input type="text" placeholder='username' onChange={onTypeUsername} value={username} required/>
			</label>
			
			<label >ROOM:
				<input type="text" placeholder='room' onChange={onTypeRoomname} required/>
			</label>
		</div>
	);
}
...
function EnterButton ({mode, onCreateRoom, onJoinRoom} : EnterButtonProps) {
	...
	if (mode === "create"){
		return <button type='submit' onClick={onCreateRoom} disabled={isServerSlept}>CREATE ROOM</button>;
	} else {
		return <button type='submit' onClick={onJoinRoom} disabled={isServerSlept}>JOIN ROOM</button>;
	}
...
function ServerStatus () {
	...
	return <div className={isServerSlept? style["server-off"] : style["server-on"]}>{isServerSlept?"Sorry, free host, server is sleeping.":"Server is awake!"}</div>;
}
	...
	return (
		<div className={style['start-form-background']} >
			<form className={style['start-form']}>
				<Logo />
				<Tabs mode={mode} onSelectCreate={()=>setMode("create")} onSelectJoin={()=>setMode("join")}/>
				<Inputs username={user.username} onTypeUsername={handleTypeUsername} onTypeRoomname={handleTypeRoomname}/>
				<EnterButton mode={mode} onCreateRoom={createRoom} onJoinRoom={joinRoom}/>
				<ServerStatus />
			</form>
		</div>
	);
