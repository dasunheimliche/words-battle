...
export const setUserTurnAndEmit = (
  userTurn: User,
  user: User,
  room: string,
  socket: SocketType,
  setUserTurn: Dispatch<User>
) => {
  socket.emit("setUserTurn", { userTurn, user, room });
  setUserTurn(userTurn);
};

export const hitHost = (
  damage: Position[] | undefined,
  socket: SocketType,
  room: string,
  damages: Record<string, number>,
  board: Board,
  host: User,
  setHost: Dispatch<User>
) => {
  if (!damage) return;
